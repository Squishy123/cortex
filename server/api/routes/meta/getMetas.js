    ...
    handler: async (req, res) => {
        try {
            ...
            if (req.headers.skip_length)
                verified = await Meta.find({ node_id: mongoose.Types.ObjectId(req.headers.node_id), reading_name: req.headers.reading_name }).skip(Number(req.headers.skip_length)).limit(50).sort({ created_at: -1 });
            else
                verified = await Meta.find({ node_id: mongoose.Types.ObjectId(req.headers.node_id), reading_name: req.headers.reading_name }).limit(50).sort({ created_at: -1 });
